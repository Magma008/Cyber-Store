                        ...
                        specialCards.map((card, index) => (
                            <div key={index} className={`card p-2 sm:p-4 flex flex-col ${card.id == 2 ? "bg-[#F9F9F9]" : card.id == 3 ? "bg-[#EAEAEA]" : card.id == 4 ? "bg-[#2C2C2C] text-white" : "bg-white"}`}>
                                ...
                                <div className="card-body flex flex-col gap-2 sm:gap-4 grow">
                                    ...
                                    <Link className='mt-auto' to={`/categories/${card?.type}`}>
                                        <button className={`border-2 text-[14px] sm:text-base mt-auto ${card.id == 4 ? "border-white" : "border-black"} rounded-md py-0.5 sm:py-2.5 px-4 sm:px-8 transition-all duration-300 hover:shadow-lg hover:shadow-gray-500 cursor-pointer hover:text-shadow-lg hover:text-shadow-gray-500 w-full`}>Shop Now</button>
                                    </Link>
                                </div>
                            </div>
                        ))
