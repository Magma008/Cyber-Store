              ...
              cards.slice(0, 5).map((item, i) => (
                <div key={i} className="card bg-[#F6F6F6] p-2 sm:p-4 md:p-6 flex flex-col gap-2 rounded-[9px] relative">
                  <div className="liked text-xl sm:text-4xl cursor-pointer text-red-500 absolute top-1 sm:top-3 right-1 sm:right-3">
                    {
                      item.isLiked ? <IoIosHeart /> : <IoIosHeartEmpty />
                    }
                  </div>
                  <Link to={`/categories/${item?.category}/${item?.id}`}>
                    <div className="card-image aspect-10/12 flex justify-center">
                      <img className='w-full h-full object-contain object-center' src={item.image} alt={item.name} />
                    </div>
                  </Link>
                  <div className="card-body flex flex-col gap-2 sm:gap-3 items-center w-full grow">
                    <h1 className="text-center font-medium text-[10px] sm:text-base">{item.name} hbfhbfhbh</h1>
                    <h3 className="text-base sm:text-2xl font-semibold">${item.price}</h3>
                    <Link to={`/categories/${item?.category}`} className='mt-auto w-full'>
